...
def detect_qpp(data, parc_ts, window_length, 
               num_scans, parallel_cores, permutations=4, 
    ...
    if parallel_cores > 0:
        ...
        pool.starmap(run_qpp_iteration, 
                     [(perm, parc_ts, window_length, trs, initial_trs,
                      df, inpectable_trs, iterations, 
                      convergence_iterations, correlation_thresholds) 
                      for perm in range(permutations)])
    else:
        permutation_result = \
        [run_qpp_iteration(perm, parc_ts, window_length, trs, initial_trs,
                           df, inpectable_trs, iterations, 
                           convergence_iterations, correlation_thresholds) 
        for perm in range(permutations)]
...
def parcellation_ts(parcellation_file, data):
    parc = nb.load(parcellation_file)
    labels = np.squeeze(parc.get_fdata())
    n_cols = labels.shape[0]
    data_short = data[:,:n_cols]
    parc_ts = np.array([data_short[:, labels==i].mean(axis=1) 
...
def run_main(input_dir, n_sub, window_length, parcellation_file,
             parallel_cores, aws_load):
    if aws_load:
        group_data, hdr = load_data_and_stack_s3(bucket_name, n_sub)
    else:
        group_data, hdr = load_data_and_stack(input_dir, n_sub)
    parc_ts = parcellation_ts(parcellation_file, group_data)
    qpp_results = detect_qpp(group_data.T, parc_ts,
                             window_length, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_directory',
                        ...
                        default='',
                        type=str)
    parser.add_argument('-c', '--parcellation_file',
                        help='<Required> Path to parcellation file ',
                        required=True,
                        type=str)
    ...
    run_main(args_dict['input_directory'], args_dict['n_sub'], 
             args_dict['window_length'], args_dict['parcellation_file'],
